...
public class DialogMenu extends Menu {
	...
	private boolean MessagesScreen;
	
	private String GameName;
	
	public DialogMenu(int Number, String Game) {
		if (Number == 0){
			MessagesScreen = false;
		}else if (Number == 1){
			MessagesScreen = true;
		}
		GameName = Game;
	...
	public void tick() {
		...
		if (MessagesScreen){
			if (input.enter.clicked){
				game.setMenu(new MainMenu(GameName));
			}
		}else{
			if (input.enter.clicked){
				game.setMenu(new NewCodeMenu());
			}
		}
	}
